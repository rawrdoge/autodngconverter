...
v2 (C++ rewrite): adds Cross-App Rotation Sync (PRD §5.3).
]]
...
-- ---------------------------------------------------------------------------
-- Cross-App Rotation Sync (PRD §5.3)
-- ---------------------------------------------------------------------------
-- Standard EXIF orientation CW cycle.
local function rotate_cw(orient)
  if orient == 1 then return 6
  elseif orient == 2 then return 5
  elseif orient == 3 then return 8
  elseif orient == 4 then return 7
  elseif orient == 5 then return 4
  elseif orient == 6 then return 3
  elseif orient == 7 then return 2
  elseif orient == 8 then return 1
  end
  return 1
end

-- POST {source_path, orientation, client_id} to the rotation-updated endpoint.
local function notify_rotation_updated(source_path, orientation)
  local base = api_base_url()
  if not base or base == "" then return end
  local token = api_token()
  local esc = source_path:gsub('"', '\\"')
  local body = string.format(
    '{"source_path":"%s","orientation":%d,"client_id":"darktable-lua"}',
    esc, orientation)
  local auth_hdr = (token ~= "") and (' -H "Authorization: Bearer ' .. token .. '"') or ""
  local cmd = string.format('curl -s -o nul -w "%%{http_code}" -X POST %s%s -H "Content-Type: application/json" -d "%s"',
    auth_hdr, base .. "/api/v1/imports/by-source/rotation-updated", body)
  local pipe = io.popen(cmd)
  if pipe then
    local code = pipe:read("*a")
    pipe:close()
    if code and code:match("200") then
      dt.print("Rotation synced → all apps (orient " .. orientation .. ")")
    else
      dt.print("WARN: rotation sync failed (" .. tostring(code) .. ")")
    end
  end
end

-- Cycle orientation for all selected images and sync to all apps.
local function sync_rotation_all()
  local images = dt.gui.action_images
  if not images or #images == 0 then
    dt.print("No images selected for rotation sync")
    return
  end
  for _, img in ipairs(images) do
    local cur = img.orientation or 1
    local next_o = rotate_cw(cur)
    pcall(function() img.orientation = next_o end)
    local src = img.path .. "/" .. img.filename
    pcall(notify_rotation_updated, src, next_o)
  end
end
...
-- JPEG into the matching DNG. Gated by `auto_reeembed_on_export` (default off).
-- ---------------------------------------------------------------------------
...
local function install()
  ...
  -- Rotation sync button (discoverable + bindable via shortcut below)
  local rot_btn = dt.new_widget("button"){
    label = "↻ Sync Rotation",
    clicked_callback = function() pcall(sync_rotation_all) end
  }
  ...
  -- Bindable keyboard shortcut: rotate selected + sync to all apps (PRD §5.3)
  pcall(function()
    dt.register_event("shortcut", function()
      pcall(sync_rotation_all)
    end, "Rotate selected + sync to all apps")
  end)

  -- Register the export-hook (gated by auto_reeembed_on_export, default off).
  pcall(function()
